...
//     if (chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start_time).count() >= timeout_threshold) {
//         cerr << RED << "Error: Timeout occurred while waiting for challenge from server." << RESET << endl;
//         return 1;
//     }

//     // If no timeout and no packet loss, simulate successful reception of challenge
//     cout << GREEN << "\nChallenge successfully received from server:" << RESET << endl;
...
//     if (!cFile) {
//         cerr << RED << "Error: Unable to write to c.txt." << RESET << endl;
//         return 1;
//     }
//     cFile << c;
//     cFile.close();
//     cout << GREEN << "Received random challenge has been saved to c.txt" << RESET << endl;

//     // Save the random value to "v.txt"
//     ofstream vFile("v.txt");
//     if (!vFile) {
//         cerr << RED << "Error: Unable to write to v.txt." << RESET << endl;
//         return 1;
//     }
//     vFile << v;
//     vFile.close();
//     cout << GREEN << "Received random value has been saved to v.txt" << RESET << endl;

//     cout << BLUE << "\nServer has sent these values for the client to use." << RESET << endl;
